...
# Create main frame with scrollbar to accommodate all elements
...
label_caminho_csv = ctk.CTkLabel(scrollable_frame, text='Caminho arquivo CSV:')
label_caminho_csv.pack(pady=5)

entry_path_csv = ctk.CTkEntry(scrollable_frame, width=400, placeholder_text="Caminho arquivo CSV")
entry_path_csv.pack(pady=5)

botao_arquivo = ctk.CTkButton(scrollable_frame, text="Selecionar arquivo CSV", command=selecionar_arquivo)
botao_arquivo.pack(pady=5)

label_caminho_csv = ctk.CTkLabel(scrollable_frame, text='Modelo de Certificado (PPTX):')
label_caminho_csv.pack(pady=5)

entry_path_pptx = ctk.CTkEntry(scrollable_frame, width=400, placeholder_text="Caminho arquivo PPTX")
entry_path_pptx.pack(pady=5)

botao_arquivo = ctk.CTkButton(scrollable_frame, text="Selecionar arquivo PPTX", command=selecionar_arquivo_pptx)
botao_arquivo.pack(pady=5)

# Font customization frames
font_customization_frame = ctk.CTkFrame(scrollable_frame)
...
# Boxes section
label_boxs = ctk.CTkLabel(scrollable_frame, text="Caixas de texto a serem substituídas")
label_boxs.pack(pady=5)

frame_listas = ctk.CTkFrame(scrollable_frame)
...
label_pasta_destino = ctk.CTkLabel(scrollable_frame, text='Pasta de destino para arquivos .PDF')
label_pasta_destino.pack(pady=5)

entry_path_folder = ctk.CTkEntry(scrollable_frame, width=400, placeholder_text="Caminho pasta de destino")
entry_path_folder.pack(pady=5)

frame_botoes_finais = ctk.CTkFrame(scrollable_frame)
